"""
Kanban HTML Agent - Primary Kanban board implementation

Uses LLM-generated column/card data with embedded CSS.
...
from services.kanban_generator import KanbanGenerator

logger = logging.getLogger(__name__)


class KanbanHtmlAgent(BaseAgent):
    """
    Primary Kanban agent using HTML/CSS rendering.

    This agent generates Kanban boards as HTML content that can be
    ...
    async def initialize(self):
        ...
        self.generator = KanbanGenerator()
        self.initialized = True
        logger.info("KanbanHtmlAgent initialized")
    ...
    async def generate(self, request: DiagramRequest) -> Dict[str, Any]:
        """
        Generate Kanban board as HTML content.
        ...
        try:
            ...
            return {
                ...
                "diagram_type": "kanban",
                "metadata": {
                    "generation_method": "kanban_html",
                    "structured_data": response.get("structured_data"),
                    "llm_model": response.get("metadata", {}).get("llm_model"),
                    "generation_time_ms": response.get("metadata", {}).get("generation_time_ms"),
                    "total_cards": response.get("metadata", {}).get("total_cards")
                }
            }

        except Exception as e:
            logger.error(f"KanbanHtmlAgent generation error: {e}", exc_info=True)
            return {
                "success": False,
                "error": str(e),
                "diagram_type": "kanban"
