"""
Gantt HTML Agent - Primary Gantt chart implementation

Uses percentage-based positioning with embedded CSS.
...
from services.gantt_generator import GanttGenerator

logger = logging.getLogger(__name__)


class GanttHtmlAgent(BaseAgent):
    """
    Primary Gantt agent using HTML/CSS rendering.

    This agent generates Gantt charts as HTML content that can be
    ...
    async def initialize(self):
        ...
        logger.info("GanttHtmlAgent initialized")
    ...
    async def generate(self, request: DiagramRequest) -> Dict[str, Any]:
        """
        Generate Gantt chart as HTML content.
        ...
        try:
            ...
            logger.error(f"GanttHtmlAgent generation error: {e}", exc_info=True)
            return {
                "success": False,
                "error": str(e),
                "diagram_type": "gantt"
