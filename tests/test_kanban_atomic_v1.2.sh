#!/bin/bash
#
# Test Script: KANBAN_BOARD v1.2.0 - Light/Dark Mode & Column Stretching
# Version: 1.2.0
# Tests: 8 configurations showcasing light/dark mode, column stretching, and position presets
#
# v1.2.0 Features Tested:
# - Light/dark theme_mode with translucent RGBA pastel backgrounds
# - Column stretching fix (flex:1 1 0, min-width:0)
# - Assignee popup in add/edit card
# - Position presets: full_content, left_two_thirds, right_two_thirds
# - Column count restrictions by position
#
...
OUTPUT_DIR="./tests/test_outputs/kanban_atomic_v1.2_${TIMESTAMP}"
...
echo ""
echo "=============================================="
echo "  KANBAN_BOARD v1.2.0 - Atomic Endpoint Test"
echo "  Light/Dark Mode & Column Stretching"
echo "=============================================="
...
echo ""
echo "--- Checking KANBAN_BOARD v1.2.0 Availability ---"
...
generate_slide() {
    ...
    local preset=$6
    local width=$7
    local start_col=${8:-""}  # Optional custom start_col
    ...
    echo "  Theme Mode: $theme_mode | Columns: $column_count | Preset: $preset"
    echo "  Size: ${width}x14 grid = ${element_width}x${element_height}px element | Margin: ${margin}px"

    # Build JSON payload with theme_mode
    local json_payload
    if [ -n "$start_col" ]; then
    ...
    # Save full response
    echo "$RESPONSE" | jq . > "$OUTPUT_DIR/${num}_${theme_mode}_response.json" 2>/dev/null

    # Extract fields
    SUCCESS=$(echo "$RESPONSE" | jq -r '.success // false')
    KANBAN_HTML=$(echo "$RESPONSE" | jq -r '.html // ""')
    RETURNED_THEME_MODE=$(echo "$RESPONSE" | jq -r '.theme_mode_used // "light"')
    ...
    if [ "$SUCCESS" = "true" ] && [ -n "$KANBAN_HTML" ] && [ "$KANBAN_HTML" != "null" ]; then
        ...
        echo "  Theme Mode: $RETURNED_THEME_MODE | Columns: $RETURNED_COLUMNS | Cards: $RETURNED_CARDS"
        ...
        # Check for column stretching (flex:1 1 0)
        if echo "$KANBAN_HTML" | grep -q "flex:1 1 0"; then
            echo -e "  ${GREEN}Column stretching: VERIFIED (flex:1 1 0)${NC}"
        else
            echo -e "  ${YELLOW}Column stretching: NOT FOUND (check CSS)${NC}"
        fi

        # Check for RGBA pastel backgrounds
        if echo "$KANBAN_HTML" | grep -q "rgba("; then
            echo -e "  ${GREEN}Translucent backgrounds: VERIFIED (RGBA)${NC}"
        else
            echo -e "  ${YELLOW}Translucent backgrounds: NOT FOUND${NC}"
        fi

        # Save Kanban HTML
        echo "$KANBAN_HTML" > "$OUTPUT_DIR/${num}_${theme_mode}_kanban.html"
        ...
        C1_SLIDE="{
            ...
            \"content\": {
                ...
                \"subtitle\": \"Theme: $theme_mode | $position_info | ${width}x14 = ${element_width}x${element_height}px\",
                \"body\": \"\",
                \"footer_text\": \"KANBAN_BOARD v1.2.0 Test\",
                \"logo\": \" \"
            }
        }"
...
# ============================================
# Generate KANBAN_BOARD Slides (8 Configs for v1.2.0)
# v1.2.0 Focus: Light/Dark mode with RGBA pastel backgrounds
# ============================================
echo "--- Generating KANBAN_BOARD v1.2.0 Slides (8 Configs) ---"
echo ""

# Light Mode Tests (4 configs)
echo -e "${CYAN}=== LIGHT MODE TESTS ===${NC}"
echo ""

# Config 1: Full Content, 4 Columns, Light Mode
generate_slide 1 "Light Mode - Full Content 4-Col" "light" 4 10 "full_content" 30

# Config 2: Full Content, 5 Columns, Light Mode
generate_slide 2 "Light Mode - Full Content 5-Col" "light" 5 10 "full_content" 30

# Config 3: Left Two Thirds, 3 Columns, Light Mode
generate_slide 3 "Light Mode - Left 2/3 3-Col" "light" 3 10 "left_two_thirds" 20

# Config 4: Right Two Thirds, 3 Columns, Light Mode
generate_slide 4 "Light Mode - Right 2/3 3-Col" "light" 3 10 "right_two_thirds" 20

# Dark Mode Tests (4 configs)
echo -e "${MAGENTA}=== DARK MODE TESTS ===${NC}"
echo ""

# Config 5: Full Content, 4 Columns, Dark Mode
generate_slide 5 "Dark Mode - Full Content 4-Col" "dark" 4 10 "full_content" 30

# Config 6: Full Content, 5 Columns, Dark Mode
generate_slide 6 "Dark Mode - Full Content 5-Col" "dark" 5 10 "full_content" 30

# Config 7: Left Two Thirds, 3 Columns, Dark Mode
generate_slide 7 "Dark Mode - Left 2/3 3-Col" "dark" 3 10 "left_two_thirds" 20

# Config 8: Right Two Thirds, 3 Columns, Dark Mode
generate_slide 8 "Dark Mode - Right 2/3 3-Col" "dark" 3 10 "right_two_thirds" 20
...
C1_REQUEST="{
    \"title\": \"KANBAN_BOARD v1.2.0 - Light/Dark Mode Test - $TIMESTAMP\",
    \"template_id\": \"L25\",
    \"slides\": [$C1_SLIDES]
}"
...
<html>
<head>
    <title>KANBAN_BOARD v1.2.0 - Light/Dark Mode Test</title>
    <style>
        * { box-sizing: border-box; }
        ...
        .kanban-frame.light-bg {
            background: #f8fafc;
        }
        .kanban-frame.dark-bg {
            background: #1f2937;
        }
        .theme-meta {
            padding: 12px 20px;
...
<body>
    <div class="header">
        <h1>KANBAN_BOARD<span class="version-badge">v1.2.0</span></h1>
        <p class="subtitle">Light/Dark Mode with Translucent RGBA Pastel Backgrounds</p>
        <a href="$C1_URL" target="_blank" class="presentation-link">View Full Presentation</a>
    </div>

    <div class="features-list">
        <div class="features-title">v1.2.0 Features Tested</div>
        <ul>
            <li><strong>Column Stretching Fix:</strong> <code>flex:1 1 0</code> + <code>min-width:0</code> for even column distribution</li>
            <li><strong>Light Mode:</strong> Translucent pastel backgrounds (60% opacity) with dark text</li>
            <li><strong>Dark Mode:</strong> Same pastel backgrounds (50% opacity) with light text</li>
            <li><strong>RGBA Backgrounds:</strong> Slide background shows through translucent columns</li>
            <li><strong>Assignee Popup:</strong> Add/Edit card prompts for both title and assignee initials</li>
        </ul>
    </div>
...
# Add light mode cards (configs 1-4)
for i in 1 2 3 4; do
    html_file="${i}_light_kanban.html"
    if [ -f "$OUTPUT_DIR/$html_file" ]; then
        case $i in
            1) name="Full Content 4-Col"; preset="full_content"; grid="30x14"; cols="4" ;;
            2) name="Full Content 5-Col"; preset="full_content"; grid="30x14"; cols="5" ;;
            3) name="Left 2/3 3-Col"; preset="left_two_thirds"; grid="20x14"; cols="3" ;;
            4) name="Right 2/3 3-Col"; preset="right_two_thirds"; grid="20x14"; cols="3" ;;
        esac
        ...
        cat >> "$OUTPUT_DIR/preview_kanban.html" << EOF
            <div class="theme-card">
                ...
                <iframe class="kanban-frame light-bg" srcdoc="$CHART_CONTENT"></iframe>
                <div class="theme-meta">
                    ...
                    <div class="meta-item">Columns: <span class="meta-value">$cols</span></div>
                </div>
            </div>
EOF
    fi
...
# Add dark mode cards (configs 5-8)
for i in 5 6 7 8; do
    html_file="${i}_dark_kanban.html"
    if [ -f "$OUTPUT_DIR/$html_file" ]; then
        case $i in
            5) name="Full Content 4-Col"; preset="full_content"; grid="30x14"; cols="4" ;;
            6) name="Full Content 5-Col"; preset="full_content"; grid="30x14"; cols="5" ;;
            7) name="Left 2/3 3-Col"; preset="left_two_thirds"; grid="20x14"; cols="3" ;;
            8) name="Right 2/3 3-Col"; preset="right_two_thirds"; grid="20x14"; cols="3" ;;
        esac
        CHART_CONTENT=$(cat "$OUTPUT_DIR/$html_file" | sed 's/"/\&quot;/g' | tr '\n' ' ')
        cat >> "$OUTPUT_DIR/preview_kanban.html" << EOF
            <div class="theme-card">
                ...
                <iframe class="kanban-frame dark-bg" srcdoc="$CHART_CONTENT"></iframe>
                <div class="theme-meta">
                    <div class="meta-item">Preset: <span class="meta-value">$preset</span></div>
                    <div class="meta-item">Grid: <span class="meta-value">$grid</span></div>
                    <div class="meta-item">Columns: <span class="meta-value">$cols</span></div>
...
cat > "$OUTPUT_DIR/test_report.json" << EOF
{
    ...
    "v1.2.0_features_tested": [
        "theme_mode parameter (light/dark)",
        "translucent RGBA pastel backgrounds",
        "column stretching (flex:1 1 0, min-width:0)",
        "light mode: dark text with 60% opacity backgrounds",
        "dark mode: light text with 50% opacity backgrounds",
        "assignee popup in add/edit card"
    ],
    "results": {
...
echo ""
echo "=============================================="
echo "  KANBAN_BOARD v1.2.0 TEST RESULTS"
echo "=============================================="
echo ""
echo "v1.2.0 Features:"
echo "  - Column stretching: flex:1 1 0 + min-width:0"
echo "  - Light mode: RGBA pastel backgrounds (60%) + dark text"
echo "  - Dark mode: RGBA pastel backgrounds (50%) + light text"
echo "  - Assignee popup in add/edit card"
echo ""
echo "Configurations Tested:"
echo -e "  ${CYAN}LIGHT MODE:${NC}"
echo -e "    1. Full Content + 4 cols    - 30x14 grid"
echo -e "    2. Full Content + 5 cols    - 30x14 grid"
echo -e "    3. Left 2/3 + 3 cols        - 20x14 grid"
echo -e "    4. Right 2/3 + 3 cols       - 20x14 grid"
echo ""
echo -e "  ${MAGENTA}DARK MODE:${NC}"
echo -e "    5. Full Content + 4 cols    - 30x14 grid"
echo -e "    6. Full Content + 5 cols    - 30x14 grid"
echo -e "    7. Left 2/3 + 3 cols        - 20x14 grid"
echo -e "    8. Right 2/3 + 3 cols       - 20x14 grid"
echo ""
echo -e "Generation: ${GREEN}$SUCCESS_COUNT${NC} / 8 success"
if [ $FAIL_COUNT -gt 0 ]; then
    echo -e "            ${RED}$FAIL_COUNT${NC} / 8 failed"
fi
echo ""
echo "Presentation:"
if [ -n "$C1_URL" ]; then
